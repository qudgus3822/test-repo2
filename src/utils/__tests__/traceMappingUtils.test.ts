...
import type { TraceMappingField, TraceMapping } from '@/types/traceability.types.js';

// ── Helpers ───────────────────────────────────────────────────────────────────

function makeField(overrides: Partial<TraceMappingField> = {}): TraceMappingField {
  return {
    key: 'value',
    label: '값',
    type: 'string',
...
describe('getNestedValue', () => {
  ...
  it('nested key', () => {
    expect(getNestedValue({ stats: { additions: 5 } }, 'stats.additions')).toBe(5);
  });
...
describe('extractItems', () => {
  it('returns array at flat path', () => {
    ...
    expect(extractItems({ mergeRequests: items }, 'mergeRequests')).toEqual(items);
  });
...
describe('extractDisplayColumns', () => {
  ...
  it('referenceFields come before itemFields', () => {
    const ref: TraceMappingField[] = [makeField({ key: 'ref1', display: true })];
    const items: TraceMappingField[] = [makeField({ key: 'item1', display: true })];
    const cols = extractDisplayColumns(ref, items);
  ...
  it('returns empty array when all fields are display:false', () => {
    ...
    const items: TraceMappingField[] = [makeField({ key: 'b', display: false })];
...
describe('formatCellValue', () => {
  ...
  it('number with unit appends unit', () => {
    expect(formatCellValue(352, makeField({ type: 'number', unit: '라인' }))).toBe('352라인');
  });

  it('float number rounded to 2 decimal places', () => {
    expect(formatCellValue(3.14159, makeField({ type: 'number' }))).toBe('3.14');
  });
  ...
  it('date string returns Korean locale format', () => {
    const result = formatCellValue('2024-01-15T10:30:00Z', makeField({ type: 'date' }));
  ...
  it('invalid date string returns original string', () => {
    expect(formatCellValue('not-a-date', makeField({ type: 'date' }))).toBe('not-a-date');
  });
  ...
  it('boolean false returns "아니오"', () => {
    expect(formatCellValue(false, makeField({ type: 'boolean' }))).toBe('아니오');
  });
  ...
  it('non-empty string returns the string', () => {
    expect(formatCellValue('hello', makeField({ type: 'string' }))).toBe('hello');
  });

  it('object type returns JSON string', () => {
    expect(formatCellValue({ a: 1 }, makeField({ type: 'object' }))).toBe('{"a":1}');
  });
});
...
describe('buildTableRows', () => {
  ...
  it('formattedKey takes precedence over raw value', () => {
    ...
    const columns: TraceMappingField[] = [
      makeField({ key: 'responseTime', label: '응답시간', type: 'number', formattedKey: 'responseTimeFormatted' }),
    ];
    const rows = buildTableRows(items, columns);
...
describe('extractSummaryValues', () => {
  it('extracts summary fields from details', () => {
    ...
    const fields: TraceMappingField[] = [
      makeField({ key: 'avgResponseTime', label: '평균 응답시간', type: 'number', unit: '초' }),
